...
test.describe("frontend orders integration", () => {
  test("creates an order from the list page and exercises detail mutations", async ({
    ...
    await page.getByRole("button", { name: "Refund Order" }).last().click();
    await expectToast(page, "Order marked as Refunded");
  });

  test("filters the list and can cancel an existing order", async ({
    page,
    scenario,
  }) => {
    await scenario.load("full-app");
